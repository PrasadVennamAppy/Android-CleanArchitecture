...
import com.fernandocejas.sample.features.recipes.RecipeCartItem
import retrofit2.Call
import javax.inject.Inject

interface RecipesRepository {
    fun recipes(): Either<Failure, List<RecipeCartItem>>

    class Network
    @Inject constructor(private val networkHandler: NetworkHandler,
                        private val service: RecipeService) : RecipesRepository {

        override fun recipes(): Either<Failure, List<RecipeCartItem>> {
            return when (networkHandler.isNetworkAvailable()) {
                true -> request(service.recipes(), { it.map { recipeEntity -> recipeEntity.toRecipe() } }, emptyList())
